...
def main():
    ...
    pv_model = np.squeeze(modelcube_with_beam[:, :, int(new_resolution/2.)])
    ...
    if plot_int_map == True:
        ...
        a = axes.pcolormesh(-xx / dist, yy / dist, model_int_map_without_beam, shading='auto', rasterized=True,
                             cmap='PuBuGn', vmin = np.nanmin(model_int_map_without_beam), 
                             vmax = np.nanmax(model_int_map_without_beam))
